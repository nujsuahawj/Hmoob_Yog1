...
class LineNotifyAdmin {
    ...
    public function WlnMenu() {
        ...
        add_menu_page(
            ...
            __('Line Notify', 'woo-line-notify'), // menu_title
        ...
        //add_submenu_page ( $this->plugin_data['TextDomain'], 'Settings', 'Settings', 'manage_options', 'admin.php?page=woo-line-notify&tab=notification_setting', '');
        add_submenu_page ( $this->plugin_data['TextDomain'], 'Debug setting', __('Debug', 'woo-line-notify'), 'manage_options', 'admin.php?page=woo-line-notify&tab=debug', '');
        add_submenu_page ( $this->plugin_data['TextDomain'], 'API Setting', __('API', 'woo-line-notify'), 'manage_options', 'admin.php?page=woo-line-notify&tab=api', '');
        add_submenu_page ( $this->plugin_data['TextDomain'], 'About', __('About', 'woo-line-notify'), 'manage_options', 'admin.php?page=woo-line-notify&tab=about', '');
    }
    /**
    ...
    public function NotificationTrigger( $order_id ) {
        ...
        // Check notify rule
        $allowStatus = json_decode( $get_option['status'] );
        
        // Logging
        file_put_contents( WOO_LINE_NOTIFY_PATH. 'logs/log.txt', json_encode(["DateTime"=>$order->order_date, 
                                                                              "order_id" => $order_id, 
                                                                              "status" => $order_data['status'], 
                                                                              "allow_status" => $allowStatus])."\n", FILE_APPEND );
        
        if( in_array( 'wc-' . $order_data['status'], $allowStatus) ) {
        ...
        switch ( $active_tab ) {
            ...
            case 'about':
                ?>
                    ...
                    <a href="https://facebook.com/9apinan" target="_blank">
                    ...
                    <a href="https://facebook.com/9apinan" target="_blank">Apinan Woratrakun</a><br>CEO of <a href="https://iotech.co.th/" target="_blank">iOTech Enterprise</a>.
                    </p>
    ...
    public function debug_callback() {
        ...
        if( isset( $option_['debug'] ) ) {
            ...
            $orderLogs = (file_exists( WOO_LINE_NOTIFY_PATH . '/logs/log.txt' )) ? file_get_contents( WOO_LINE_NOTIFY_PATH . '/logs/log.txt' ) : '';
            printf( "<p>". __('Order status logs', 'woo-line-notify') ." &darr;</p><textarea class='large-text' readonly style='height: 300px;display: block;margin-top: 10px;background-color: #fff;'>%s</textarea>", $orderLogs );
        }
    }
    ...
    public function endpoint_callback() {
        ...
        printf(
            '<input class="regular-text" type="text" name="api-endpoint" id="api-endpoint" value="%s" required>', $endpoint
        );
    }
